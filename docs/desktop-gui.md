...
| **回测分析** | 参数区（策略、标的池、区间、初始资金、基准、手续费/滑点）、净值 vs 基准、回撤曲线、月度收益、绩效指标、成交流水、期末持仓 | 选好参数点「开始回测」；在成交流水里核对每笔买卖与费用 |
...
* **体积参考**：Windows onedir 目录约 10–20 MB（zip 约 10–15 MB），onefile 约 15–20 MB；
  已 `excludes` 掉 flask/jinja2/pytest/numpy/pandas/matplotlib。
