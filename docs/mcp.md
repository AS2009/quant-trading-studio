...
* **tools（41 个）**：行情、盘口/逐笔/资金流、**L2 工具箱**（大单/分时/封板/扫描/排行）、自选池、策略读写与校验、回测、持仓账本、模拟盘、系统状态；
...
> 下表以 `backend/quantstudio/mcp/tools_*.py` 实际注册的工具为准（共 **41** 个）。
...
| 工具 | 作用 | 写操作 |
|---|---|---|
| `level2_orderbook` | 五档盘口 + 委比/委差/价差 + 能力协商（免费源 5 档；十档需付费源或本地导入） | 只读 |
| `level2_ticks` | 最近逐笔成交 + 多空统计（默认 60 条，文本里注明覆盖上限与第三方方向口径） | 只读 |
| `capital_flow` | 四档资金流（超大单/大单/中单/小单）与主力净额（由逐笔单笔成交额分档自算） | 只读 |
| `l2_big_orders` | 大单追踪（单笔金额 ≥ 阈值，默认 100 万；可按方向过滤） | 只读 |
| `l2_flow_series` | 资金流分时（分钟聚合 + 累计净额 + 四档分档） | 只读 |
| `l2_seal_status` | 封板状态（涨停/跌停、封单量与封成比、距涨停） | 只读 |
| `l2_scan` | 盘口异动扫描（缺省=自选池，最多 10 只，按委比降序） | 只读 |
| `l2_flow_rank` | 资金流排行（缺省=自选池，按主力净额降序；约 10–25 秒/只） | 只读 |
