...
- **腾讯** `qt.gtimg.cn` / `web.ifzq.gtimg.cn` / `proxy.finance.qq.com`：实时快照 + **历史 K 线（可回溯到 2001 年，约 6400 根）**（默认历史备份源）+ 行业板块（含板块涨跌家数）+ **五档盘口与逐笔成交**（唯一提供逐笔方向标记的免费源）；
...
- **数据**：乘法前复权日线（`adjust=qfq` = 后复权缩放到最新真实价）——腾讯自带的「前复权」是减法复权，
  深历史会出现负价与失真的涨跌幅（实测 600519 取 5000 根时首根 -304.77 元），所以本项目统一按后复权口径取数、
  再缩放到最新价（末根 = 今日真实价，收益率与后复权一致）；按标的共同交易日对齐，停牌当日不做交易；
...
| `KLINE_DAYS` / `MAX_KLINE_DAYS` | `250` / `5000` | 看板默认 / 单次最多 K 线根数（5000 ≈ 20 年；腾讯实测最多约 6400 根 / 2001 年起） |
| `LOG_LEVEL` | `INFO` | 日志级别 |
